...
def track_pre(coll, particles):
    ...
    if not xc.bdsim.engine.assert_ready_to_track_or_skip(
...
def track_core(coll, part):
    import xcoll as xc
    #xc.bdsim.engine._link.clearData() # Clear the old data - bunch particles and hits TODO needed?

    #send_to_bdsim = (part.state == HIT_ON_GEANT4) | (part.state == HIT_ON_GEANT4_SEC)
    ...
    #num_sent = send_to_bdsim.sum()
    num_sent = len(part.state)
    #idx_sent = np.arange(len(part.x))[send_to_bdsim]
    idx_sent = np.arange(num_sent)

    if xc.bdsim.engine.reentry_protection_enabled:
        ### TODO remove this part after geant4 bug fixed
        ...
        print('reenetry protection to be implemented for BDSIM engine')
    else:
        bdsim_id = xc.bdsim.engine._element_index[coll.geant4_id]
        xc.bdsim.engine._link.TrackXSuite(bdsim_id, coll.geant4_id, part, 0)
    ...
    returned_dead = products['state'] == LOST_WITHOUT_SPEC # TODO add [:num_sent]
    ...
    returned_alive = products['state'] > 0 # TODO add [:num_sent] 
    ...
    new_p = part.filter(idx_returned_alive)
